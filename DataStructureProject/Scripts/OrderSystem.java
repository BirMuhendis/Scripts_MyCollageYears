...
public class OrderSystem 
{
    ...
    public boolean AddOrder(String order[], Node root, int index, boolean siblingSearch)
    {
        ...
        root = (root == null) ? this.root : root; // if ==null so get root as root value or get this.root
        if(this.root == null){
            this.root = newNode;
            root = newNode;
        }
    ...
    public void print(Node root, String space)
    {
        ...
        System.out.print(space + "└──" + root.get_data() + " (" + root.get_quantity() + ")" + " [P(" + (root.get_parent_node() == null ? "null" : root.get_parent_node().get_data())  +
         ") " + "| S(" + (root.get_sibling_node() == null ? "null" : root.get_sibling_node().get_data())  + ")]\n");
